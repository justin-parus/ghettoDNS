...
class GhettoDNS:
  '''Simple DNS Server work-around. Uses free "ip services" and emails user when IP changes'''
  ...
  def mail_ip(self):
    ...
    try:
      ...
      if self.last_ip != ip:
        ...
        email_addr = self.email_usr.get_username() + '@' + self.email_url
        ...
        with smtplib.SMTP(self.mail_server) as server:
          server.ehlo()
          server.starttls()
          server.login(self.email_usr.get_username(), self.email_usr.get_password())
